<template>
        ...
        <div class="max-w-md mb-6">
          <span class="text-xs text-gray-400 tracking-wider">{{productData.productName}} #{{productData.id}}</span>
          <h2 class="mt-6 mb-4 text-5xl md:text-7xl lg:text-8xl font-heading font-medium">{{productData.productName}}</h2>
          <p class="flex items-center mb-6">
            <span class="mr-2 text-sm text-purple-500 font-medium">RM</span>
            <span class="text-3xl text-purple-500 font-medium">{{productData.productPrice}}</span>
          </p>
          <p class="text-lg text-gray-400">The nulla commodo, commodo eros a lor, tristique lectus. Lorem sad .</p>
        </div>
        ...
        <div class="mb-10">
          ...
          <input class="w-24 px-3 py-2 text-center bg-white border-2 border-purple-500 outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50 rounded-xl" type="text" placeholder="1">
        </div>
        <div class="flex flex-wrap -mx-2 mb-12">
          <button-add-cart :product="productData"></button-add-cart>
...
<script>
import ButtonAddCart from '../Cart/ButtonAddCart.vue'
export default {
    name:"product-info",
    props: ['product'],
    ...
    data () {
    return {
      productData: this.product, // you cannot use the same name as declared for props
    }
